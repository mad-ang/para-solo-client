...
export default class Network {
    ...
    async createCustom(roomData: IRoomData) {
        const { name,userId, description, password, autoDispose } = roomData;
        ...
        this.initialize();
        let allUserList :string[]= [];
        this.room?.state.players.forEach(element => {
            console.log("userId => ",
            element.userId)
            allUserList.push( element.userId )
        }
        );
    ...
    initialize() {
        ...
        this.myUserId = this.room.userId;
        // find all userId in the map


        store.dispatch(setSessionId(this.room.sessionId));
        store.dispatch(setUserId(this.room.userId));
        this.webRTC = new WebRTC(this.mySessionId, this);
        // new instance added to the players MapSchema
        this.room.state.players.onAdd = (player: IPlayer, key: string) => {
            console.log("누구누구 있니? => ",key);
            console.log("player => ", this.room?.state.players.get(key));
            ...
            player.onChange = (changes) => {
                changes.forEach((change) => {
                    ...
                    if (field === "name" && value !== "") {
                        ...
                        store.dispatch(
                            setPlayerUserIdMap({ id: key, userId: value })
                        )
